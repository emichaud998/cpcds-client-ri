...
class ApplicationController < ActionController::Base
  ...
  attr_accessor :explanationofbenefits, :practitioners, :patients, :locations, :organizations, :practitionerroles,
                :coverages, :resources, :fhir_explanationofbenefits, :fhir_practitioners, :fhir_patients, :fhir_organizations, :fir_coverages, :fhir_locations, :patient_resources, :patient, :eob, :eobs
  ...
  def load_fhir_eobs(patientid, eobid = nil)
    ...
    unless (200..206).member?(results.response[:code])
    ...
    explanationofbenefits = fhir_explanationofbenefits.map do |eob|
      Eob.new(@client, eob, patients, practitioners, locations, organizations, coverages,
              practitionerroles)
    end.sort_by { |a| -a.sortDate }
  ...
  def get_fhir_resources(fhir_client, type, resource_id, patient_id = nil)
    search = if patient_id.nil?
               { parameters: { _id: resource_id } }
             else
               { parameters: { _id: resource_id, patient: patient_id } }
             end
  ...
  def connect_to_server(code = nil)
    ...
    if @client.present?
      if !!session[:is_auth_server?]
        ...
        @client.set_bearer_token('SAMPLE_TOKEN')
      end
    else
  ...
  def get_new_token(code)
    ...
    begin
      ...
      # reset_session
      redirect_to home_path, alert: "Failed to connect: #{e.message}" and return
    end
  ...
  def capture_search_query(results)
    ...
    @search = URI.decode_www_form_component(results.request[:url]) if results.request[:url].present?
  end
